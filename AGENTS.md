...
**Two modes.** The OpenAlma layer is gated by the memU review status (`get_memu_review_status`, backed by `state.memu_session`): **standalone** keeps Atomic fully self-contained (local knowledge, local conversations); **integrated (OpenAlma) mode** takes knowledge from the owner's memU memory via mcp-memu-server instead of a local brain, while conversations and transcripts stay in Atomic, scoped to one exact owner + soul. In integrated mode, local setup/config and export routes answer 403 (`routes/setup.rs`), and WebSocket/chat access validates owner+soul scope against mcp per request (`ws.rs` `validate_scope`, conversation storage filters in `storage/sqlite/chat.rs`). Conversations from before the owner existed have no recoverable identity and stay sealed in integrated mode (deliberate; never guess-stamp them).

**Soul workspaces.** In integrated mode, each exact owner+soul is bound to one local Atomic database (SQLite only). The binding is a registry setting (`state.rs` `bind_workspace_database`/`resolve_workspace_core`, keyed by `workspace_setting_key(user_id, soul_id)`), resolved per HTTP/WS request and enforced for database selection; integrated manual database management is hidden/blocked. The UI runs two views over one soul: **Memories** (memU memories, categories, dossiers) and **Workspace** (the soul's local atoms/notes/wiki/chats/tags — upstream behavior behind an explicit source switch, `routes/memu_proxy.rs` `workspace_requested`). Search federates both: integrated search routes through mcp by default (`routes/search.rs`), the palette previews five per section with incremental bottom-of-dropdown loading for more. memU's shared search caps requests at 20 — semantic/hybrid stay capped; do not disguise a larger fixed cap as all-results paging. Conversations and chat-pipeline events retain the workspace DB scope; sealed (pre-owner) conversations stay out of soul search.

**Dossiers in Atomic.** Dossier members carry member/cited badges; deletion is disabled for memories whose dossier prose currently cites them and Delete focuses the cited `[M#]` (`AtomReader.tsx` `memoryIsCited`). Only current prose citations block — member-only links delete normally (the citation check and batch hard-delete share one SQLite transaction on the mcp side). Approvals and Entities are singleton routed tabs (`MainView.tsx`).

**Reader lifecycle.** Every open memU reader stays mounted, CSS-hidden while its tab is inactive, so switching tabs never discards local drafts or scroll position (`MainView.tsx`; hidden readers suppress keyboard/shared actions through the existing `active` guards). Memory mutations carry an expected-summary baseline — when the server's summary has moved on, mcp answers an exact 409, the stale tab is kept as a read-only retired snapshot (`AtomReader.tsx` `retired`), and current state opens independently; newly opened readers bypass the detail cache so they cannot show stale server state. Singleton Approvals/Entities tabs follow the same keep-mounted rule, so following a dossier/entity link never refetches and loses local state.
