...
export const COMMAND_MAP: Record<string, CommandSpec> = {
  ...
  merge_memu_entities: {
    method: 'POST',
    path: (a) => `/api/memu/entities/${encodeURIComponent(a.id as string)}/merge`,
    argsMode: 'body',
    transformArgs: (a) => ({ duplicate_entity_id: a.duplicateId }),
  },
  ...
  end_memu_session: {
    ...
    argsMode: 'body',
    transformArgs: (a) => ({ conversation_id: a.conversationId }),
  },
